...
| 阶段一账本快照（员工账号测试前） | PASS | 10 个任务且活动任务 0；52 条日志；渠道/管理员已用均为 `97672169` quota = ¥195.344338；Key A/B 已用合计与其完全一致。后续双员工验收后的最终快照见第 13 节 |
...
### 11.1 阶段一方舟账本基线（员工账号测试前）

本地成功任务时间窗口为 2026-08-31 16:40:52 至 2026-09-01 00:14:59（Asia/Shanghai）。方舟控制台账单可能延迟；数据生成后按相同模型和尽量相同时间窗口核对下表，不得把本表自行标记成“官方已结算”。

| 分辨率 | 成功任务数 | actual usage | 本地最终费用 |
| --- | ---: | ---: | ---: |
| 480p | 2 | 96,874 tokens | ¥6.781180 |
| 720p | 5 | 1,149,300 tokens | ¥80.451000 |
| 1080p | 3 | 1,950,075 tokens | ¥108.112158 |
| **合计** | **10** | **3,196,249 tokens** | **¥195.344338** |

本地 `tasks.quota` 合计、渠道 `used_quota`、管理员 `used_quota` 与两把 Key 的 `used_quota` 合计均为 `97672169` raw quota，即 ¥195.344338。官方核对时至少比较成功任务数、模型、actual tokens 和人民币金额；若方舟按小时归集导致时间边界差异，应先按任务 ID/提交时间解释，不能自动改写本地历史账务。

### 11.2 2026-09-01 方舟正式账单核对

2026-09-01 方舟正式账单的筛选合计应付金额为 **¥74.94**。同一 Asia/Shanghai 自然日内，New API `tasks` 权威账本为 8 个成功任务、1,223,812 actual tokens、¥74.958862。

| 核对项 | 方舟 | New API | 结论 |
| --- | ---: | ---: | --- |
| actual usage | 1,223,812 tokens | 1,223,812 tokens | 完全一致 |
| 人民币金额 | ¥74.94 | ¥74.958862 | 相差 ¥0.018862，约 0.0252% |

按需求方允许几分钱金额差的验收口径，2026-09-01 单日官方账单核对 **PASS**。该结果验证公司级总量；由于多个虚拟 Key 共用一把真实方舟 Key，上游账单不能直接验证员工级拆分。2026-08-31 没有单独的官方人民币截图，需求方于 2026-09-01 明确接受该日已有的官方 Token 与本地账本证据，并同意不再补充金额截图。因此本轮账单与 Token 准确性验收记为 **CLOSED / PASS**。

## 12. 最终 GO / NO-GO

**账单与 Token 准确性：GO / CLOSED。** 本机文生视频的 actual usage、最终人民币计费、跨员工归属和公司级总量均已达到当前验收标准。2026-09-01 的官方 Token 与金额已核对；2026-08-31 的单独金额截图由需求方明确豁免，不再构成阻塞。图生视频当前明确延后，不纳入本轮结论；真实异步失败仍为 `not reproducible safely`，首笔自然失败发生后补录，但不影响本次账单准确性收口。

## 13. 双员工账号 + 独立虚拟 Key 真实验收

**测试时间：** 2026-09-01 01:18 至 01:57（Asia/Shanghai）。

**完整记录：** [employee-account-key-acceptance-2026-09-01.md](./employee-account-key-acceptance-2026-09-01.md)。该文件逐项记录为什么测试、如何测试、5 个真实任务、人民币对账、跨员工隔离、额度不足拦截、页面检查、源码诊断、结果位置和保留现场；此处只保留权威摘要。

### 13.1 架构和核心结果

创建普通用户 `poc-emp-a-708606`（ID 3）和 `poc-emp-b-708606`（ID 4），各分配 ¥100 用户额度；分别创建模型限制为 Seedance 2.5 的 unlimited 虚拟 Token 17/18。员工只使用虚拟 Key，二者共用既有方舟渠道 #1 和同一个真实方舟 Key。

| 员工 | 真实成功任务 | 最终已用 | 最终剩余 | 结果 |
| --- | ---: | ---: | ---: | --- |
| A | 3 | ¥30.579086 | ¥69.420914 | 用户、Token、任务和日志归属一致 |
| B | 2 | ¥21.294000 | ¥78.706000 | 用户、Token、任务和日志归属一致 |
| 合计 | 5 | ¥51.873086 | — | 渠道增量与其完全一致，Root 账本不变 |

双向跨员工任务查询均为 404，自查均为 200；B 余额不足的合法 1080p/30 秒请求得到本地 403，任务数、渠道用量、用户余额不变。管理员 `/users`、`/dashboard/users` 和 `/usage-logs/common` 均能看到员工级数据。

### 13.2 五个真实任务和最终总账

| 员工 | 参数 | ratio | 任务 ID | actual | 最终费用 | 结果 |
| --- | --- | --- | --- | ---: | ---: | --- |
| A | 480p/5s | 默认 | `task_yoa1stQY05MdwjgQ1zfQKaZEKQeTV5A3` | 48,437 | ¥3.390590 | PASS，退 ¥0.000056 |
| B | 720p/4s | 默认 | `task_NTpy5UdIv5E2skgFKNDI5wZoSljH0Phh` | 87,300 | ¥6.111000 | PASS |
| A | 1080p/5s | 默认 | `task_BKXor39DAm7D0DJeKAavGdD3N89N3NqS` | 245,388 | ¥13.604310 | 最终对账 PASS；预扣少 ¥0.020124 |
| B | 720p/10s | 默认 | `task_fBf7j7XDoOPjapD6i4QDwJeiH6w08Bju` | 216,900 | ¥15.183000 | PASS |
| A | 1080p/5s | 16:9 | `task_09zsDbqLrci678WH7td8WRtFnQZSeTNN` | 245,025 | ¥13.584186 | PASS，预扣与最终相等 |

加入本轮后的最终账本基线为：15 个成功任务、4,039,299 actual tokens、本地最终费用 ¥247.217424；`tasks.quota` 与渠道 `used_quota` 均为 `123608712` raw quota。第 11.1 节的 10 任务/¥195.344338 是本轮之前的阶段一快照，不再是当前最终值。

### 13.3 新发现且未擅自修复的问题

1. 默认自适应比例的一笔 1080p/5 秒任务 actual 为 245,388 tokens，高于 245,025 tokens 预估，最终准确补扣 ¥0.020124。显式 `16:9` 对照任务回到 245,025 tokens。最终账单准确，但若用户钱包必须是绝对硬上限，需要固定比例或增加安全预扣。
2. 通用日志页面明细正确，但顶部历史“用量”显示 ¥0。源码定位为 `SumUsedQuota` 的第二次 RPM/TPM 扫描覆盖了第一次 quota 汇总。
3. `quota_data` 没有反向计入 480p 的 28 raw quota 退款：员工 A 聚合比任务净额多 ¥0.000056；当前全局两笔同类退款使用户统计聚合比任务/渠道权威账本多 ¥0.000112。
4. unlimited token 会继续累计 used 并显示负 remain；实际额度边界是用户钱包。这是当前数据模型行为，不影响调用，但运维时应查看用户额度。

### 13.4 当前决策

“每个员工一个普通用户账号 + 一把独立虚拟 Key + 共用一个真实方舟渠道”已通过核心可行性验收，建议作为约 20 人公司内部使用方案。账单与 Token 准确性不再是生产阻塞项；若以后要把用户余额当作绝对不能透支一分钱的严格停机线，仍需单独处理默认比例的预扣边界并复测，这属于额度控制问题，不推翻最终账单准确性结论。
